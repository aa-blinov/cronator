...
async def test_artifact_lifecycle(test_client: AsyncClient, tmp_artifacts_dir):
    ...
    try:
        ...
        # 3. Wait for finish
        if not await wait_for_script_finish(test_client, script_id):
        ...
        # 6. Delete artifact
        response = await test_client.delete(f"/api/executions/{execution_id}/artifacts/{artifact_id}")
        assert response.status_code == 200
...
async def test_binary_artifact_support(test_client: AsyncClient, tmp_artifacts_dir):
    ...
    try:
        await test_client.post(f"/api/scripts/{script_id}/run")
        await wait_for_script_finish(test_client, script_id)

        # Get execution status to find ID
        resp = await test_client.get(f"/api/executions?script_id={script_id}")
        execution_id = resp.json()["items"][0]["id"]
